...
const { autoSearchConfigService } = require('../services/scheduler/autoSearchConfigService');
const { autoSearchRunner } = require('../services/scheduler/autoSearchRunner');
const { autoSearchExporter } = require('../services/scheduler/autoSearchExporter');
...
router.get('/config', async (req, res, next) => {
  try {
    const config = await autoSearchConfigService.getConfig();
    res.json(config);
  } catch (err) {
    next(err);
  }
...
router.put('/config', async (req, res, next) => {
  try {
    const updated = await autoSearchConfigService.updateConfig(req.body);
    res.json(updated);
  } catch (err) {
    next(err);
  }
...
router.post('/pause', async (req, res, next) => {
  try {
    const config = await autoSearchConfigService.toggleEnabled(false);
    res.json({
...
router.post('/resume', async (req, res, next) => {
  try {
    const config = await autoSearchConfigService.toggleEnabled(true);
    res.json({
...
router.post('/run', async (req, res, next) => {
  try {
    const result = await autoSearchRunner.triggerRun('manual');
    res.status(201).json({
...
router.get('/runs/:runId/export', async (req, res, next) => {
  try {
    const { runId } = req.params;
    const xlsx = await autoSearchExporter.exportRunResultsToXlsx(runId);
